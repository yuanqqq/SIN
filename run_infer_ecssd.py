...
parser.add_argument('--data_dir', metavar='DIR', default='/media/yuanqing/ssd/code/visual_tracking/SO/Saliency-Objectness-master/Data/SRC',help='path to images folder')
parser.add_argument('--pretrained', metavar='PTH', help='path to pre-trained model', default= './pretrain_ckpt/model_best.tar')
parser.add_argument('--output', metavar='DIR', default= './output/ecssd/200' ,help='path to output folder')
...
def test(model, img_paths, save_path, idx):
    ...
    H_, W_  = int(np.ceil(H*0.7/16.)*16-15), int(np.ceil(W*0.7/16.)*16-15)
...
def main():
    ...
    tst_lst = glob(args.data_dir + '/*.' + 'jpg')
    tst_lst.sort()
