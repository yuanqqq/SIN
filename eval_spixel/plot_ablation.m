...
full_res_path = './ablation/full_model/bsd/';
num_list = [54 96 150 216 294 384 486 600 726 864 1014 1176 1350 1536 1944]; 
n_set = length(num_list);
full = zeros(n_set,5);
for i=1:n_set
    load_path = [full_res_path  num2str(num_list(i)) '/results.csv']; 
    full(i,:) = loadcsv(load_path);
end

concat_res_path = './ablation/concat_weighted/';
num_list = [54 96 150 216 294 384 486 600 726 864 1014 1176 1350 1536 1944]; 
n_set = length(num_list);
concat = zeros(n_set,5);
for i=1:n_set
    load_path = [concat_res_path  num2str(num_list(i)) '/results.csv']; 
    concat(i,:) = loadcsv(load_path);
end

noweight_res_path = './ablation/noconcat_noweight/';
num_list = [54 96 150 216 294 384 486 600 726 864 1014 1176 1350 1536 1944]; 
n_set = length(num_list);
noweight = zeros(n_set,5);
for i=1:n_set
    load_path = [noweight_res_path  num2str(num_list(i)) '/results.csv']; 
    noweight(i,:) = loadcsv(load_path);
end

con_noweight_res_path = './ablation/concat_noweight/';
num_list = [54 96 150 216 294 384 486 600 726 864 1014 1176 1350 1536 1944]; 
n_set = length(num_list);
con_noweight = zeros(n_set,5);
for i=1:n_set
    load_path = [con_noweight_res_path  num2str(num_list(i)) '/results.csv']; 
    con_noweight(i,:) = loadcsv(load_path);
end
...
h = zeros(1,1); 
figure;
h(4)=plot(full(:,1),full(:,2), 'k-','MarkerFaceColor','k', 'linewidth',2); hold on; 
h(3)=plot(concat(:,1),concat(:,2), 'r-', 'MarkerFaceColor','r','linewidth',2); hold on;
h(2)=plot(noweight(:,1), noweight(:,2), 'g-', 'MarkerFaceColor','g','linewidth',2); hold on;
h(1)=plot(con_noweight(:,1),con_noweight(:,2), 'b-','MarkerFaceColor','m', 'linewidth',2 ); hold on;
...
% h(2)=plot(etps_save(:,1),etps_save(:,2), 'g-x', 'MarkerSize',8); hold on; 
...
%           'Location','southeast');
% lg = legend(h,'full model', 'baseline model + weight loss', 'baseline model + no concat', 'baseline model','Location','southeast');
lg = legend(h, 'baseline model', 'baseline model + no concat', 'baseline model + weight loss', 'full model', 'Location','southeast');
