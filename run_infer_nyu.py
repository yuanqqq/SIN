...
parser.add_argument('--data_dir', metavar='DIR', default='./nyu_test_set/nyu_preprocess_tst', help='path to images folder')
...
parser.add_argument('--output', metavar='DIR', default= './output/NYU/finetune' , help='path to output folder')
...
random.seed(100)
@torch.no_grad()
def test(args, model, img_paths, save_path, idx,scale):
      # Data loading code
    ...
    spixel_bd_image = mark_boundaries(given_img_np / np.max(given_img_np), spixel_label_map.astype(int),
                                        color=(1, 1, 1))
...
def main():
    ...
    # for scale in [0.9]:
        ...
        args.input_img_height, args.input_img_width = input_img_height * scale-15, input_img_width * scale-15
        ...
        for n in range(len(tst_lst)):
          time = test(args, model, tst_lst, save_path, n,scale)
          mean_time += time
